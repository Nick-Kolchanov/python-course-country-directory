...
from settings import API_KEY_NEWS, NEWS_COUNT_PER_COUNTRY
...
class NewsClient(BaseClient):
    ...
    async def get_news(self, alpha2code: str) -> Optional[dict]:
        """
        Получение данных о новостях.

        :param alpha2code: Двухбуквенный код страны
        :return:
        """

        return await self._request(
            f"{await self.get_base_url()}?country={alpha2code.lower()}&apiKey={API_KEY_NEWS}&pageSize={NEWS_COUNT_PER_COUNTRY}"
        )
